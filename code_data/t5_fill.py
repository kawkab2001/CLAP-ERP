...
def plot_loss(training_logs):
    ...
    plt.savefig('t5_train_loss.png')  # Save the plot
    plt.show()
...
def test_model(model, tokenizer, masked_sentence):
    """
    Test the fine-tuned T5 model on a masked sentence with multiple <MASK> tokens.
    Args:
...
def evaluate_metrics(model, tokenizer, dataset):
    ...
    for i in range(len(example)):
        ...
        true_embeddings = model.encoder.embed_tokens.weight[
            [tokenizer.convert_tokens_to_ids(tokenizer.tokenize(word))[0] for word in actual_words]
        ].detach().cpu().numpy()
        predicted_embeddings = model.encoder.embed_tokens.weight[
